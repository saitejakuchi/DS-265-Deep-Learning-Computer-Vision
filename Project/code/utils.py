...
class MulticoilForwardPIDMRI(nn.Module):
    def __init__(self, orientation):
        self.orientation = orientation
        super(MulticoilForwardPIDMRI, self).__init__()
        return
...
def get_aliasing_mask(acs_lines=26, total_lines=384, R=1):
    x1 = np.random.rand()
    return mask
...
def saveimages(samples, loc, R, normalize=False):
    samples = samples + torch.rand_like(samples) * (0.00001*R)
    torchvision.utils.save_image(
        samples,
        loc,
        nrow=int(samples.shape[0] ** 0.5),
