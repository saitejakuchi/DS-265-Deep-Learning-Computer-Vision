...
class LangevinOptimizer(torch.nn.Module):
    ...
    def _sample(self, y, index):
        ...
        # ksp_data = ref[0].cpu().detach().numpy()
        # maps_data = maps[0].cpu().detach().numpy()

        # np.save('ksp_data.npy', ksp_data)
        # np.save('maps_data.npy', maps_data)
        # sys.exit()
            ...
            for c in pbar:
                ...
                if self.config['save_images']:
                    ...
                    if (c+1) % self.config['save_iter'] ==0 :
                        # print(f'{step_size = } {c = } {self.sigmas[c]} {self.sigmas[-1]}')
                        # print(f'Index for saving {index=}')
                        # np.save(f'file_{index}.npy', samples.cpu().detach().numpy())
                        # import pdb;pdb.set_trace()
    ...
    def _sample1(self, y, index):
        ...
        F = sp.linop.FFT((ref.shape[0], ref.shape[1], ref.shape[2]), axes=(-1, -2))
        P = sp.linop.MatMul((ref.shape[0], mask.shape[1], ref.shape[2]), mask[np.newaxis, :, :])
        A = P * F * S
        with torch.no_grad():
    ...
    def sample(self, y, index):
        self._initialize()
        mvue = self._sample(y, index)
...
def mp_run(rank, config, project_dir, working_dir, files):
    ...
    if config['anatomy'] == 'knees':
        ...
        dataset = MVU_Estimator_Brain(files,
                                ...
                                pattern=config['pattern'],
                                orientation=config['orientation'],
                                mask_type=config['mask_type']) #Added mask type for psuedoinverse formulation
    else:
    ...
    for index, sample in enumerate(tqdm(loader)):
        ...
        for batch_idx in range(config['batch_size']):
            idx = sample["slice_idx"][batch_idx].item()
            exp_name = sample['mvue_file'][batch_idx].split('/')[-1] + '|langevin|' + f'slide_idx_{idx}'
        ...
        print(f'Before sampling. {index=}')
        outputs = langevin_optimizer.sample((ref, mvue, maps, mask), idx)
