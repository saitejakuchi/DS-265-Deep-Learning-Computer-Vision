...
class Conv2d(torch.nn.Module):
    def __init__(self,
        ...
        if gated:
            self.gate_weight = torch.nn.Parameter(weight_init(
                [out_channels, in_channels, kernel, kernel], **init_kwargs) * init_weight) if kernel else None
    ...
    def forward(self, x):
        ...
        if self.gated:
            gw = self.gate_weight.to(
                x.dtype) if self.gate_weight is not None else None
        ...
        if self.fused_resample and self.up and w is not None:
            x = torch.nn.functional.conv_transpose2d(x, f.mul(4).tile(
                [self.in_channels, 1, 1, 1]), groups=self.in_channels, stride=2, padding=w_pad+f_pad)
            if self.gated:
            ...
            if self.gated:
                gate_output = torch.sigmoid(
                    torch.nn.functional.conv2d(x, gw, padding=w_pad + f_pad))
            ...
            if w is not None:
                if self.gated:
                    gate_output = torch.sigmoid(
                        torch.nn.functional.conv2d(x, gw, padding=w_pad))
...
class UNetBlock(torch.nn.Module):
    def __init__(self,
        ...
        self.conv0 = Conv2d(in_channels=in_channels, out_channels=out_channels, kernel=3,
                            up=up, down=down, resample_filter=resample_filter, gated=gated, **init)
        ...
        self.conv1 = Conv2d(in_channels=out_channels,
                            out_channels=out_channels, kernel=3, gated=gated, **init_zero)
        ...
        if self.num_heads:
            ...
            self.qkv = Conv2d(in_channels=out_channels, out_channels=out_channels*3,
                              kernel=1, gated=gated, **(init_attn if init_attn is not None else init))
            self.proj = Conv2d(
                in_channels=out_channels, out_channels=out_channels, kernel=1, gated=gated, **init_zero)
...
# Missing key(s) in state_dict: "enc.128x128_block0.skip.weight", "enc.128x128_block0.skip.bias", "enc.128x128_block0.skip.gate_weight".
