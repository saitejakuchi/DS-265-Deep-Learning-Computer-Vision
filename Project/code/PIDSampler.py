...
class LangevinOptimizer(torch.nn.Module):
    def __init__(self, config, logger, project_dir='./', experiment=None):
        ...
        # Load our trained NCSN model
        # configs = importlib.import_module(f"configs.ve.custom_config")
        # config = configs.get_config()

        # # create model and load checkpoint
        # self.score_model = mutils.create_model(config)
        # ema = ExponentialMovingAverage(self.score_model.parameters(),
        #                             decay=config.model.ema_rate)
        # state = dict(step=0, model=self.score_model, ema=ema)
    ...
    def _sample(self, y):
        ...
        forward_operator = lambda x: MulticoilForwardPIDMRI(self.config['orientation'])(torch.complex(x[:, 0], x[:, 1]), maps, batch_mri_mask)
        ...
        with torch.no_grad():
            for c in pbar:
                ...
                for s in range(n_steps_each):
                    ...
                    # p_grad = self.score_model(samples, labels)
    ...
    for index, sample in enumerate(tqdm(loader)):
        ...
        for batch_idx in range(config['batch_size']):
            ...
            if config['save_images']:
                file_name = f'{exp_name}_R={config["R"]}_final_generated_sample_{batch_idx}.jpg'
                saveimages(estimated_mvue[batch_idx:batch_idx+1].abs().flip(-2), file_name, config['R'], normalize=True)
                if experiment is not None:
                    experiment.log_image(file_name)

                file_name = f'{exp_name}_input_{batch_idx}.jpg'
